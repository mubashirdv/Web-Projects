...
// Get the course name from URL parameters
const urlParams = new URLSearchParams(window.location.search);
const courseName = urlParams.get('course');

// Check if the course exists
if (!courses[courseName]) {
    alert("Course not found!");  // Alert if the course does not exist
...
// Initialize quiz title
document.getElementById("quizTitle").textContent = courseName;

// Function to display the current question
function displayQuestion() {
    const questionContainer = document.getElementById("questionContainer");

    // Calculate progress based on the current question index and total questions
    const progress = ((currentQuestionIndex + 1) / questions.length) * 100; // +1 to account for the current question being displayed
    ...
    if (currentQuestionIndex < questions.length) {
        ...
        questionContainer.innerHTML = `
            ...
            ${question.options.map((option, index) => `
                <div>
                    <input type="radio" name="answer" id="option${index}" value="${option}" ${selectedAnswers[currentQuestionIndex] === option ? 'checked' : ''}>
